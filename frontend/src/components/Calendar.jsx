...
const MyCalendar = ({ selectedService, onAddToBasket }) => {
      ...
      return (
        ...
        !reservations.some(
          (r) =>
            ...
            parseLocalDateTime(r.appointmentDate).toLocaleTimeString([], {
              hour: "2-digit",
              minute: "2-digit",
              hour12: false,
            }) === selectedHour &&
  ...
  useEffect(() => {
    if (selectedService) {
      fetch(
        `/api/provider/availability/by-service?serviceName=${encodeURIComponent(
  ...
  const availableDays = availabilities
    .map((a) => parseLocalDateTime(a.availableDate))
    .filter(Boolean);

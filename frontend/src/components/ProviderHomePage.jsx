...
export function ProviderHomePage() {
  ...
  const handleAddTimeSlot = async () => {
    ...
    if (
      timeSlots.some(
        (slot) =>
          slot.availableDate.startsWith(dateStr.slice(0, 16)) &&
    ...
    <Layout>
      ...
      >
        <section style={{ flex: 1 }}>
          <div
            className="provider-section-title"
            style={{ fontSize: "1.5rem", color: "#4B6000", marginBottom: 20 }}
          >
            You are a provider in service:{" "}
            <span style={{ fontWeight: "bold" }}>
              {selectedServiceNames.join(", ") || "None"}
            </span>
          </div>
          <hr
            style={{
              border: "none",
              borderTop: "1.5px solid #dbe6c2",
              margin: "0 0 24px 0",
            }}
          />
          <div
          ...
          {showServiceSelector && (
            <div
              className="provider-card"
              style={{ marginTop: 18, background: "#f6faee", color: "#4B6000" }}
            >
              <div style={{ fontWeight: "bold", marginBottom: 8 }}>
                Select your services and subservices
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                ...
                {subServices.map((sub) => (
                  <button
                    ...
                    className={`provider-tag ${
                      selectedServices.includes(sub.name) ? "selected" : ""
                    }`}
                  >
                    {sub.name}
                  </button>
        ...
        <aside className="provider-calendar" style={{ minWidth: 340 }}>
          <div
            className="provider-section-title"
            style={{ fontSize: "1.2rem", color: "#4B6000", marginBottom: 10 }}
          >
            Add Availability
          </div>
          {selectedServices.length > 1 && (
            <select
              ...
              onChange={(e) =>
                setSelectedServiceForAvailability(e.target.value)
              }
          ...
          >
            ...
            <div style={{ margin: "15px 0" }}>
              <label style={{ fontWeight: 500, color: "#4B6000" }}>
                Select a time:
              </label>
              <div
                style={{
